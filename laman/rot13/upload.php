...
<section class="bg-light h-auto" id="start">
    <div class="container" >
        <div class="row">
            ...
            if (isset($_POST['enkripsi'])) {
                ...
                if (empty($filename or $lokasifile)) {
                    echo "<script>window.alert('Mohon File Di Masukkan!');
                                window.location = 'index.php?page=13upload#hasil'</script>";
            ...
            $input = "INSERT INTO rot13(id, metode, waktu, tgl, file_en) 
            VALUES ('','Rot13 Enkripsi','$jam_sekarang', '$tanggal', '$file1')";
            mysqli_query($konek, $input);

        } elseif (isset($_POST['deskripsi'])) {
            $allowed = array('doc', 'docx', 'txt'); //untuk memasukan & menambahkan ekstensi file
            $filename = $_FILES['file']['name'];
            $lokasifile = $_FILES['file']['tmp_name'];
            $tipefile = $_FILES['file']['type'];
            $ext = pathinfo($filename, PATHINFO_EXTENSION);
            ...
            $tanggal = date("Y-m-d");
            $konek = mysqli_connect('127.0.0.1', 'root', '', 'kripto');
            $input = "INSERT INTO rot132(id, metode, waktu, tgl, file_de) 
            VALUES ('','Rot13 Deskripsi','$jam_sekarang', '$tanggal', '$file1')";
            mysqli_query($konek, $input);
        }
        ?>
            </section>